...
from ..db import menus_repository, submenus_repository
from api.servises import menus_servises

not_menu = JSONResponse(content={"detail": "submenu not found"}, status_code=404)
def create_submenu(session: Session, target_menu_id: str, data: Dict[str, str]) -> Dict[str, str]:
    ...
    if menu:
        new_submenu = submenus_repository.create_submenu_in_db(session, target_menu_id, title, description)
        return {
...
def show_submenu_by_id(session: Session, target_menu_id: str, target_submenu_id: str) -> List[Dict[str, str]]:
    menu = menus_servises.check_menu(session, target_menu_id)
...
def update_submenu_by_id(session: Session, target_menu_id: str, target_submenu_id: str, data: Dict[str, str]) -> Dict[str, str]:
    ...
    if menu:
        ...
        if submenu:
            update_submenu = submenus_repository.update_submenu_by_id_in_bd(session, submenu, data)
...
def delete_submenu_by_id(session: Session, target_menu_id: str, target_submenu_id: str) -> List[Dict[str, str]]:
    menu = menus_servises.check_menu(session, target_menu_id)
