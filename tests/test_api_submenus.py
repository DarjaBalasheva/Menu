...
async def test_create_submenu(api_client, created_menu_id):
    response = await api_client.post(f"{BASE_URL}/{created_menu_id}/submenus", json=SAMPLE_SUBMENU)
...
async def test_show_submenu(api_client, created_menu_id):
    ...
    response = await api_client.get(f"{BASE_URL}/{created_menu_id}/submenus/{submenu_id}")
    assert response.status_code == 200
    data = response.json()
    assert "id" in data
    assert data["title"] == SAMPLE_SUBMENU["title"]
