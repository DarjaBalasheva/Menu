...
async def test_create_dish(api_client, created_menu_id, created_submenu):
    ...
    response = await api_client.post(
        f'{BASE_URL}/{created_menu_id}/submenus/{submenu_id}/dishes', json=SAMPLE_DISHES
    )
...
async def test_show_all_dishes(api_client, created_menu_id, created_submenu):
    ...
    response = await api_client.get(
        f'{BASE_URL}/{created_menu_id}/submenus/{submenu_id}/dishes'
    )
...
async def test_show_dish(api_client, created_menu_id, created_submenu):
    ...
    response = await api_client.get(
        f'{BASE_URL}/{created_menu_id}/submenus/{submenu_id}/dishes/{dishes_id}'
    )
    assert response.status_code == 200
    data = response.json()
    assert 'id' in data
