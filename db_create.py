...
USERNAME = os.environ.get('db_user_login')  # Получаем значение переменной окружения 'db_host'
PASSWORD = os.environ.get('db_user_password')  # Получаем значение переменной окружения 'db_user_password'
DBNAME = os.environ.get('db_name')  # Получаем значение переменной окружения 'db_name'

engine = create_engine(f'postgresql://{USERNAME}:{PASSWORD}@localhost/{DBNAME}', echo=True)
...
# Вспомогательное ограничение для того, чтобы блюдо не могло находиться в 2-х подменю одновременно
